	...
	<div class="content">
		...
		<div class="innerOuter">
			...
			<form:form modelAttribute="board"
				action="${contextPath }/board/update/${boardCd}/${boardNo}"
				id="enrollForm" method="post" enctype="multipart/form-data">
				<table align="center">
					<tr>
						<th>제목</th>
						<td><form:input path="boardTitle" cssClass="form-control"
					...
					<c:if test="${boardCd ne 'P' }">
						<tr>
							...
							<td><input type="file" id="upfile" class="form-control"
								name="upfile"> ${board.imgList[0].originName } <input
								type="hidden" name="imgNo" value="${empty img0no ? 0 : img0no }" />
							</td>
						</tr>
					</c:if>
					<c:if test="${boardCd eq 'P'}">
						<tr>
							...
							<td><img class="preview" src="${img0 }"> <input
								type="file" name="upfile" class="form-control inputImage"
								accept="images/*" id="img1"> <span class="delete-image"
								data-no="${img0no}">&times;</span> <input type="hidden"
								name="imgNo" value="${empty img0no ? 0 : img0no }" /></td>
						</tr>
						<tr>
							<th><label for="image">업로드 이미지2</label></th>
							<td><img class="preview" src="${img1 }"> <input
								type="file" name="upfile" class="form-control inputImage"
								accept="images/*" id="img2"> <span class="delete-image"
								data-no="${img1no}">&times;</span> <input type="hidden"
								name="imgNo" value="${empty img1no  ? 0 : img1no}" /></td>
						</tr>
						<tr>
							<th><label for="image">업로드 이미지3</label></th>
							<td><img class="preview" src="${img2 }"> <input
								type="file" name="upfile" class="form-control inputImage"
								accept="images/*" id="img3"> <span class="delete-image"
								data-no="${img2no}">&times;</span> <input type="hidden"
								name="imgNo" value="${empty img2no  ? 0 : img2no}" /></td>
						</tr>
						<tr>
							<th><label for="image">업로드 이미지4</label></th>
							<td><img class="preview" src="${img3 }"> <input
								type="file" name="upfile" class="form-control inputImage"
								accept="images/*" id="img4"> <span class="delete-image"
								data-no="${img3no}">&times;</span> <input type="hidden"
								name="imgNo" value="${empty img3no ? 0 : img3no}" /></td>
						</tr>
					</c:if>
					<tr>
						<th>내용</th>
						<td><form:textarea path="boardContent" rows="10"
								cssClass="form-control" id="content" style="resize:none;"
								required="required" /></td>
					</tr>
				</table>
				<!--
